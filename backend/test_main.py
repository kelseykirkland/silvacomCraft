...
def test_route_fail():
    response = app.test_client().get('/notexist')
...
def test_getWeather_Munich():
    response = app.test_client().get('/weather/Munich')
...
def test_getWeather_Venice():
    response = app.test_client().get('/weather/Venice')
    res = json.loads(response.data.decode('utf-8')).get("location")
    assert response.status_code == 200
...
def test_getWeather_incorrectCity():
    response = app.test_client().get('/weather/NotExist')
